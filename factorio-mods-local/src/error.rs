...
pub enum ErrorKind {
	...
	#[error_chain(custom)]
	#[error_chain(display = r#"|path: &::std::path::Path, _| write!(f, "Could not parse the ZIP file {}", path.display())"#)]
	#[error_chain(cause = "|_, err| err")]
	Zip(::std::path::PathBuf, ::zip::result::ZipError),

	/// A zipped mod has no files and is thus malformed
	#[error_chain(custom)]
	#[error_chain(display = r#"|path: &::std::path::Path| write!(f, "The zipped mod file {} is empty", path.display())"#)]
	...
	#[error_chain(custom)]
	#[error_chain(display = r#"|| write!(f, "The local Factorio installation could not be found")"#)]
	DataPath,
